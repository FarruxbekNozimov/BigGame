...
function Instance(handlebars) {
  ...
  self.cache = {};
  self.__express = middleware.bind(this); // queue for partials registration

  self._queue = null; // DEPRECATED, kept for backwards compatibility
...
Instance.prototype.registerPartials = function (directory, options, done) {
  ...
  if (this._queue) {
    self._queue.unshift(self.registerPartials.bind.apply(self.registerPartials, [this].concat(Array.prototype.slice.call(arguments))));
  ...
  w.on('file', function (root, stat, done) {
    ...
    fs.readFile(filepath, 'utf8', function (err, data) {
      if (!err) {
        ...
        var name = path.relative(directory, filepath).slice(0, -extname.length).replace(/\\/g, '/');
