...
(function () {
  ...
  var fs = require('fs'),
      ...
      TypeEmitter = require('./node-type-emitter'); // 2010-11-25 jorge@jorgechamorro.com
  ...
  function create(pathname, cb) {
    ...
    function walk() {
      fs.readdir(curpath, function (err, files) {
        if (err) {
          emitter.emit('directoryError', curpath, {
            error: err
          }, noop); //emitter.emit('error', curpath, { error: err });
        ...
        var fnodeGroups = TypeEmitter.createNodeGroups(); // TODO could allow user to selectively stat
        ...
        files.forEachAsync(function (cont, file) {
          ...
          fs.lstat(curpath + path.sep + file, function (err, stat) {
            ...
            if (err) {
              stat.error = err; //emitter.emit('error', curpath, stat);
