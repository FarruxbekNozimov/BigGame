...
function loadStrings(opts, callback) {
  ...
  _neoAsync["default"].map(strings, function (string, callback) {
    if (string !== '-') {
...
function loadFiles(opts, callback) {
  ...
  _neoAsync["default"].whilst(function () {
    return queue.length;
  }, function (callback) {
    ...
    _fs["default"].stat(path, function (err, stat) {
      if (err) {
        return callback(new Handlebars.Exception("Unable to open template file \"".concat(path, "\"")));
      }

      if (stat.isDirectory()) {
        opts.hasDirectory = true;

        _fs["default"].readdir(path, function (err, children) {
          ...
          children.forEach(function (file) {
            ...
            if (extension.test(childPath) || _fs["default"].statSync(childPath).isDirectory()) {
              queue.push({
                template: childPath,
                root: root || path
              });
...
module.exports.cli = function (opts) {
  ...
  if (opts.map) {
    _fs["default"].writeFileSync(opts.map, output.map, 'utf8');
  }

  output = output.code;

  if (opts.output) {
    _fs["default"].writeFileSync(opts.output, output, 'utf8');
  } else {
    console.log(output);
  }
};
